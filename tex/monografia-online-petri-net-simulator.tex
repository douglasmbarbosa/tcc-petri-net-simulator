...
referência: \url{https://math.stackexchange.com/questions/190111/how-to-check-if-a-point-is-inside-a-rectangle}

As transições e nome de elementos podem ser caracterizados como retângulos dentro da área \textbf{canvas}. Ao clicar na área \textbf{canvas} obtemos as posições \textbf{x} e \textbf{y} do clique. Para determinarmos se o clique ocorreu dentro de algum dos retângulos verifica-se e comparara-se o ponto \textbf{x} e \textbf{y} do clique com o ponto inferior esquerdo do retângulo. Faz-se a verificação da seguinte forma:

\begin{enumerate}
	\item $mouseX >= transitionX$;
	\item $mouseX <= transitionX + transitionWidth$;
	\item $mouseY >= transitionY$;
	\item $mouseY <= transitionY + transitionHeigth$.
\end{enumerate}

Caso todas essas comparações sejam verdadeiras, pode-se dizer que o clique ocorreu dentro de algum retângulo.
...
\begin{figure}[h!]
	...
	\end{tikzpicture}
	\caption{Retângulo no plano XY com os vértices nomeados $A$, $B$, $C$ e $D$.}
\end{figure}
